#!/bin/bash -l
#SBATCH --job-name=ccrl_1g_velpeek_d01
#SBATCH --output=phase1g_velpeek_d0p01.%j.out
#SBATCH --error=phase1g_velpeek_d0p01.%j.err
...
# Phase-1g budget sweep — d=0.01 peek. See slurm_phase1g_velpeek_d0p05.sh for
# full context. Sole CLI difference vs d=0.05 peek: --cost_budget_d 0.05 → 0.01.
...
"$PYTHON" train.py \
    ...
    --wandb_group phase1g_velpeek_d0p01 \
    --track True

EXIT_CODE=$?
echo "===== DONE d=0.01 peek  $(date)  exit=$EXIT_CODE ====="
exit $EXIT_CODE
