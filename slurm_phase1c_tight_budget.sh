#!/bin/bash -l
#SBATCH --job-name=ccrl_phase1c_tight
#SBATCH --output=phase1c_tight.%j.out
#SBATCH --error=phase1c_tight.%j.err
...
# ═════════════════════════════════════════════════════════════════════════════
#  PHASE-1c  TIGHT-BUDGET VARIANT  (d = 0.05, all else identical to 1b)
# ─────────────────────────────────────────────────────────────────────────────
#  Submit ONLY IF the Phase-1b overnight (group `phase1b_smoke`) lands in
#  Scenario 1: λ̃ ≡ 0 throughout because Ĵ_c stays below d = 0.15 at every
#  epoch.  In that regime the PID-Lagrangian dual is inactive — not from a
#  bug, but because the CMDP is feasible for the exploration policy and the
#  constraint has no teeth.  Phase-1b validates the *pipeline*; it does NOT
#  validate the dual's *responsiveness*.
#
#  Rationale for d = 0.05 specifically:
#    The dense-grid sweep (outputs/scale_check.py) gives d_wall percentiles
#    p5=0.15, p10=0.35 on the open interior.  The hard-indicator ε_hard=0.1
#    fires at ~5% of interior positions under a uniform policy.  With
#    budget d=0.05, a policy that wanders freely will have Ĵ_c ≈ 0.05
#    persistently — putting it right at the edge of the feasible set.
#    Any systematic drift toward walls lifts Ĵ_c above d, triggering the
#    PID I-channel.  This is the regime where the dual mechanism is
#    stress-tested: λ̃ lifts → cost-penalty term in the actor loss
#    activates → policy retreats → λ̃ decays → cycle.  C3 becomes
#    informative, not a "feasible CMDP" tautology.
#
#  Expected outcome (all five criteria under scenario 2):
#    C1  mean_step_cost ∈ [0.10, 0.35] (unchanged — cost shape didn't change)
#    C2  cost descends ≥ 10 % rel (driven by PID pressure, which now fires)
#    C3  λ̃ transiently ∈ (0, 50) for a nonzero fraction of epochs
#    C4  train/eval calibration (unchanged)
#    C5  no NaN, |actor_loss| < 1e4 (with responsive PID, the v1a-style
#          CRL divergence channel is clamped)
#
#  Every other hyperparameter is locked at the Phase-1b values.  The single-
#  knob perturbation means a C3 pass/fail difference vs. Phase-1b pinpoints
#  the budget as the cause — clean attribution.
# ═════════════════════════════════════════════════════════════════════════════
...
"$PYTHON" train.py \
    ...
    --wandb_group       phase1c_tight_budget \
    --track             True
