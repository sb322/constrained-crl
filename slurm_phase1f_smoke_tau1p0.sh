#!/bin/bash -l
#SBATCH --job-name=ccrl_1f_smoke_tau1p0
#SBATCH --output=phase1f_smoke_tau1p0.%j.out
#SBATCH --error=phase1f_smoke_tau1p0.%j.err
...
# ── Smoke: 3 epochs, wall budget 1h, expected ~25 min. If this runs clean
#    where the full Phase-1f (τ=0.1) NaN'd, τ=0.1 is the blow-up driver.
...
echo ""
echo "===== PHASE-1f SMOKE τ=1.0 LAUNCH  $(date) ====="

# This is a τ-bisection smoke. train.py default is tau=0.1 (Phase-1f). We
# override on the CLI with --tau 1.0 so the ONLY delta from the failed full
# Phase-1f run is the temperature; row-L2 stays on in both loss fns,
# obs-normalization stays on, ν_f stays at 1.0. 3 epochs, ~25 min GPU.
...
"$PYTHON" train.py \
    ...
    --wandb_group       phase1f_smoke_tau1p0 \
    --track             True

EXIT_CODE=$?
echo ""
echo "===== PHASE-1f SMOKE τ=1.0 DONE  $(date)  exit=$EXIT_CODE ====="
exit $EXIT_CODE
